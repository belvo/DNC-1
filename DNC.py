...
deep mind 최신 논문
differentiable neural computer 구현
https://github.com/yos1up/DNC - 원본 오픈 소스
gpu 버전이 아니라서 gpu 버전 구현함 with 승훈이형
...
def onehot(x, n):
    ret = xp.zeros(n).astype(np.float32)
    ret[x] = 1.0
    return ret
...
def C(M, k, beta):
    ...
    for i in range(M.data.shape[0]):
        ret_list[i] = overlap(F.reshape(F.select_item(F.transpose(M), Variable(cuda.to_gpu(xp.array([i] * W), _gpu_id))), (1, W)), k) * beta  # pick i-th row
...
def u2a(u):  # u, a: (N * 1) Variable
    N = len(u.data)
    conv_list = np.asarray(u.data.reshape(N).tolist())
    phi = np.argsort(conv_list)  # u.data[phi]: ascending

    a_list = [0] * N

    cumprod = Variable(cuda.to_gpu(np.array([[1.0]]).astype(np.float32), _gpu_id))
    for i in range(N):
        a_list[phi[i]] = cumprod * (1.0 - F.reshape(F.select_item(F.transpose(u), Variable(cuda.to_gpu(np.array([phi[i]]), _gpu_id))), (1, 1)))
        cumprod *= F.reshape(F.select_item(F.transpose(u), Variable(cuda.to_gpu(np.array([phi[i]]), _gpu_id))), (1, 1))
...
class DNC(Chain):
    def __init__(self, X, Y, N, W, R):
        ...
        super(DNC, self).__init__(
            ...
            #             l_Wr = L.Linear(self.R * self.W, self.Y,bias=Variable(cuda.to_gpu(0))) # nobias=True ?
            l_Wr=L.Linear(self.R * self.W, self.Y, bias=0).to_gpu(_gpu_id)  # nobias=True ?
            #             l_Wr = L.Linear(self.R * self.W) # nobias=True ?
    ...
    def __call__(self, x):
        ...
        self.M = self.M * (Variable(cuda.to_gpu(np.ones((self.N, self.W)).astype(np.float32), _gpu_id)) - F.matmul(self.ww, self.e)) + F.matmul(self.ww,
                                                                                                                                                self.v)  # N * W
        self.p = (1.0 - F.matmul(Variable(cuda.to_gpu(np.ones((self.N, 1)).astype(np.float32), _gpu_id)), F.reshape(F.sum(self.ww), (1, 1)))) \
                 * self.p + self.ww  # N * 1
        ...
        for i in range(self.R):
            self.cr_list[i] = C(self.M, F.reshape(F.select_item(F.transpose(self.kr), Variable(cuda.to_gpu(np.array([i] * self.W), _gpu_id))), (1, self.W)),
    ...
    def reset_state(self):
        ...
        self.u = Variable(cuda.to_gpu(np.zeros((self.N, 1)).astype(np.float32), _gpu_id))
        self.p = Variable(cuda.to_gpu(np.zeros((self.N, 1)).astype(np.float32), _gpu_id))
        self.L = Variable(cuda.to_gpu(np.zeros((self.N, self.N)).astype(np.float32), _gpu_id))
        self.M = Variable(cuda.to_gpu(np.zeros((self.N, self.W)).astype(np.float32), _gpu_id))
        self.r = Variable(cuda.to_gpu(np.zeros((1, self.R * self.W)).astype(np.float32), _gpu_id))
        self.wr = Variable(cuda.to_gpu(np.zeros((self.N, self.R)).astype(np.float32), _gpu_id))
        self.ww = Variable(cuda.to_gpu(np.zeros((self.N, 1)).astype(np.float32), _gpu_id))
